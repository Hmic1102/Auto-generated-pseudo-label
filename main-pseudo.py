...
parser.add_argument('--pseudo', default=False,
                    help='Set to True if working with pseudo labeled dataset')
...
def main():
    ...
    if args.csvFile and args.pseudo:
        ...
        #pseudo_policy=args.csvFile.split('_')[2]
        pseudo_policy=args.task_name.split('_LAVH')[0]
        print(pseudo_policy)
...
def save_checkpoint(state, is_best, filename='checkpoint_-1_0-1-1-1.pth.tar'):
    torch.save(state,'checkpoint_-1_0-1-1-1.pth.tar' )
    if is_best:
        shutil.copyfile('checkpoint_-1_0-1-1-1.pth.tar', 'model_best_-1_0-1-1-1.pth.tar')
